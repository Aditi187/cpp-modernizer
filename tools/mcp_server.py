...
logger.info("[WORKING] System initialized: Logging and Propts Ready.")
...
def new_cpp_parser():
    ...
    if CppParser is None:
        ...
        raise RuntimeError("CppParser unavailable")
...
ALLOWED_ROOT = str(Path.cwd().resolve())

logger.info(f"[WORKING] MCP Server Root: {ALLOWED_ROOT}")
...
try:
    ...
    logger.info("[WORKING] Infrastructure initialized: Context and ModelClient Ready.")
except Exception as e:
    logger.error(f"[ERROR] Failed to initialize project infrastructure: {e}")
    MODEL_BRIDGE = None
...
def ensure_safe_path(
    ...
    if root not in resolved.parents and resolved != root:
        ...
        raise ValueError(f"Sandbox violation: {path}")
...
def read_code(
    ...
    try:
        path = ensure_safe_path(file_path)
    except Exception as e:
        logger.error(f"[ERROR] read_code failed safe path check: {e}")
        return make_result(
            "error",
            message=str(e),
        )
    ...
    if not os.path.isfile(path):
        logger.error(f"[ERROR] read_code: file not found: {path}")
        return make_result(
            "error",
    ...
    try:
        ...
        logger.info(f"[WORKING] read_code successful: {path} ({len(text)} chars)")
        return make_result(
            "success",
            content=text,
        )
...
def write_code(
    ...
    try:
        ...
        logger.error(f"[ERROR] write_code failed safe path check: {e}")
        return make_result(
            "error",
            message=str(e),
        )
    ...
    try:
        ...
        logger.info(f"[WORKING] write_code successful: {path} ({len(content)} bytes)")
        return make_result(
            "success",
...
def run_compiler(
    ...
    if working_directory:
        ...
        try:
            ...
            cwd = ensure_safe_path(working_directory)
    ...
    if not result["ok"]:
        logger.error(f"[ERROR] run_compiler failed: {result['message']}")
        return make_result(
            "error",
    ...
    logger.info(
        f"[{'WORKING' if status == 'success' else 'ERROR'}] run_compiler finished with code {result['returncode']}"
    )
...
def run_binary(
    ...
    try:
        ...
        binary_path = ensure_safe_path(path)
    ...
    status = "success" if result.returncode == 0 else "error"
    logger.info(
        f"[{'WORKING' if status == 'success' else 'ERROR'}] run_binary finished with code {result.returncode}"
    )
...
def search_code(
    ...
    for path in glob.glob(
        ...
        if any(d in path for d in IGNORED_DIRS):
...
@mcp_server.tool()
def modernize_cpp_file(file_path: str, output_path: str | None = None) -> str:
    """
    ...
    try:
        ...
        logger.info(f"[WORKING] MCP: Initiating full modernization for {file_path}")
        final_state = run_modernization_workflow(
            code=code, source_file=abs_path, output_path=output_path
        )
...
def warmup():
    ...
    logger.info("[WORKING] Warming up server nodes...")

    if PARSER_AVAILABLE:
        try:
            new_cpp_parser().parse_string("int main(){}")
            logger.info("[WORKING] CppParser verified.")
        except Exception as e:
            logger.error(f"[ERROR] CppParser check failed: {e}")

    if MODEL_BRIDGE:
        try:
            status, msg = MODEL_BRIDGE.check_health()
            if status:
                logger.info(f"[WORKING] Model bridge healthy: {msg}")
            else:
                logger.error(f"[ERROR] Model bridge unhealthy: {msg}")
        except Exception as e:
            logger.error(f"[ERROR] Model health check crashed: {e}")


if __name__ == "__main__":
    logger.info("[WORKING] Starting Air-Gapped MCP Server...")
    warmup()
    mcp_server.run()
