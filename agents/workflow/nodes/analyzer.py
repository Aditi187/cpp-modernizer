...
def analyzer_node(state: ModernizationState) -> ModernizationState:
    ...
    Uses CppParser for deep AST analysis with timeout protection.
    """
    ...
    parser = CppParser()
    project_map = {}
    parse_result = [None]  # Use list to capture result from thread
    
    def parse_with_timeout():
        try:
            parse_result[0] = parser.parse_string(code)
        except Exception as e:
            logger.error(f"[analyzer] AST parsing failed: {e}")
            parse_result[0] = {}
    
    # Run parsing in a separate thread with 30-second timeout
    parse_thread = threading.Thread(target=parse_with_timeout, daemon=True)
    parse_thread.start()
    parse_thread.join(timeout=30)  # Wait max 30 seconds
    
    if parse_thread.is_alive():
        logger.warning("[analyzer] AST parsing timeout after 30 seconds. Using minimal analysis.")
        project_map = {}
    else:
        project_map = parse_result[0] or {}
    ...
    state["functions_info"] = list(functions_info.values())
